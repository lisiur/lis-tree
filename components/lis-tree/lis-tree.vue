			...
			<div class="uni-tree-item" :key="getId(item)">
				<div class="uni-tree-item-self">
					<div class="uni-tree-item-checkbox-wrapper" v-if="showCheckbox" @click="handleToggleCheck(currentLevelData[index])">
						<image class="uni-tree-item-checkbox-icon" :src="`/static/checked.png`" v-if="item._checked"></image>
						<image class="uni-tree-item-checkbox-icon" :src="`/static/indeterminate.png`" v-else-if="item._indeterminate"></image>
						<image class="uni-tree-item-checkbox-icon" :src="`/static/unchecked.png`" v-else></image>
					</div>
					<span class="uni-tree-item-name" @click="handleToggleExpand(currentLevelData[index])">{{item.name}}</span>
					<image class="uni-tree-item-expand-icon" :src="`/static/expand_${item._expand ? 1 : 0}.png`" v-if="item._hasChildren"></image>
				</div>
